  ...
  methods: {
    renderOption() {
      ...
      return (
        <div class={optionClass} onMouseenter={this.handleMouseEnterOption} data-id={node.id}>
          {this.renderArrow()}
          {this.renderLabelContainer([this.renderCheckboxContainer([this.renderCheckbox()]), this.renderLabel()])}
        </div>
      );
    },
    ...
    renderArrow() {
      ...
      if (/*node.isLeaf && */ instance.hasBranchNodes) {
        if (!arrowPlaceholder) arrowPlaceholder = <div class="vue-treeselect__option-arrow-placeholder">&nbsp;</div>;
    ...
    renderSubOptions() {
      ...
      return node.children.map(childNode => <vue-treeselect--option node={childNode} key={childNode.id} />);
    },
  ...
  render() {
    ...
    return (
      <div class={listItemClass}>
        {this.renderOption()}
        {node.isBranch ? <div>{this.renderSubOptionsList()}</div> : ''}
      </div>
    );
  },
};
